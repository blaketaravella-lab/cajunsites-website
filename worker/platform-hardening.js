...
async function usage(env,{prospectId=null,customerId=null,provider,operation,model=null,cacheHit=false,durationMs=null,usageData=null}){
  if(!env.DB)return;
  try{await env.DB.prepare(`INSERT INTO provider_usage_events (prospect_id,customer_id,provider,operation,model,cache_hit,duration_ms,request_count,usage_json,created_at) VALUES (?,?,?,?,?,?,?,1,?,CURRENT_TIMESTAMP)`).bind(prospectId,customerId,provider,operation,model,cacheHit?1:0,durationMs,usageData?JSON.stringify(usageData):null).run()}catch{}
}

async function upsertJob(env,{key,prospectId=null,customerId=null,type,status,stage=null,error=null,incrementAttempt=false}){
  if(!env.DB)return;
  try{await env.DB.prepare(`INSERT INTO platform_jobs (job_key,prospect_id,customer_id,job_type,status,stage,attempts,last_error,started_at,completed_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?, ?,CASE WHEN ?='Running' THEN CURRENT_TIMESTAMP ELSE NULL END,CASE WHEN ? IN ('Complete','Failed') THEN CURRENT_TIMESTAMP ELSE NULL END,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP) ON CONFLICT(job_key) DO UPDATE SET status=excluded.status,stage=excluded.stage,attempts=platform_jobs.attempts+?,last_error=excluded.last_error,started_at=CASE WHEN excluded.status='Running' AND platform_jobs.started_at IS NULL THEN CURRENT_TIMESTAMP ELSE platform_jobs.started_at END,completed_at=CASE WHEN excluded.status IN ('Complete','Failed') THEN CURRENT_TIMESTAMP ELSE NULL END,updated_at=CURRENT_TIMESTAMP`).bind(key,prospectId,customerId,type,status,stage,incrementAttempt?1:0,error,status,status,incrementAttempt?1:0).run()}catch{}
}

async function prospect(env,id){return env.DB?.prepare('SELECT * FROM prospects WHERE id=? LIMIT 1').bind(id).first()||null}
...
async function handleResearch(request,env,id){
  ...
  const key=await identityCacheKey(p),started=Date.now(),jobKey=`research:${id}`;
  await upsertJob(env,{key:jobKey,prospectId:id,type:'research',status:'Running',stage:'Researching',incrementAttempt:true});
  if(!force){
    const cached=await env.DB.prepare(`SELECT * FROM research_cache WHERE cache_key=? AND expires_at>CURRENT_TIMESTAMP LIMIT 1`).bind(key).first();
    if(cached){
      const research=parseResearch(cached.payload_json);await promoteCachedResearch(env,p,research);
      await usage(env,{prospectId:id,provider:'research_cache',operation:'business_research',cacheHit:true,durationMs:Date.now()-started,usageData:{source_count:cached.source_count}});
      await upsertJob(env,{key:jobKey,prospectId:id,type:'research',status:'Complete',stage:'Cached'});
  ...
  if(response.ok&&payload?.research){
    ...
    await env.DB.prepare(`INSERT INTO research_cache (cache_key,business_name,city,state,phone,payload_json,identity_confidence,source_count,created_at,refreshed_at,expires_at) VALUES (?,?,?,?,?,?,?, ?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,?) ON CONFLICT(cache_key) DO UPDATE SET payload_json=excluded.payload_json,identity_confidence=excluded.identity_confidence,source_count=excluded.source_count,refreshed_at=CURRENT_TIMESTAMP,expires_at=excluded.expires_at`).bind(key,p.business_name,p.city||null,p.state||null,p.phone||null,JSON.stringify(research),research.identity_confidence||null,sources,expiry(SOURCE_TTLS.research_days)).run();
    await usage(env,{prospectId:id,provider:'openai',operation:'business_research',model:env.OPENAI_RESEARCH_MODEL||'default',durationMs:Date.now()-started,usageData:{source_count:sources}});
    await upsertJob(env,{key:jobKey,prospectId:id,type:'research',status:'Complete',stage:'Researched'});
  }else await upsertJob(env,{key:jobKey,prospectId:id,type:'research',status:'Failed',stage:'Research',error:payload?.error||`HTTP ${response.status}`});
  return response;
}
...
async function handleBuild(request,env,id){
  ...
  const user=await actor(request,env);if(!user)return json({ok:false,error:'Authentication required.'},401);
  const p=await prospect(env,id);if(!p)return json({ok:false,error:'Prospect not found.'},404);
  const {env:rEnv,plan}=routedEnv(env,p,request),started=Date.now(),jobKey=`concept:${id}`;
  await upsertJob(env,{key:jobKey,prospectId:id,type:'concept_build',status:'Running',stage:'Building',incrementAttempt:true});
  const response=await appWorker.fetch(request,rEnv);const payload=await response.clone().json().catch(()=>null);
  let attempts=0;try{const row=await env.DB.prepare(`SELECT COUNT(*) AS count FROM concept_images WHERE prospect_id=? AND created_at>=datetime('now','-1 day')`).bind(id).first();attempts=Number(row?.count||0)}catch{}
  await usage(env,{prospectId:id,provider:'openai',operation:'concept_build',model:payload?.image_pipeline?.model||env.OPENAI_IMAGE_MODEL||'default',durationMs:Date.now()-started,usageData:{image_attempt_records:attempts,source_plan:plan}});
  if(plan.google_places)await usage(env,{prospectId:id,provider:'google_places',operation:'visual_inspiration',durationMs:null,usageData:{reason:plan.reason}});
  await upsertJob(env,{key:jobKey,prospectId:id,type:'concept_build',status:response.ok?'Complete':'Failed',stage:response.ok?'Deployed':'Build',error:response.ok?null:payload?.error||`HTTP ${response.status}`});
  return response;
}
...
async function dashboardSummary(request,env){
  const user=await actor(request,env);if(!user)return json({ok:false,error:'Authentication required.'},401);if(!env.DB)return json({ok:false,error:'Database unavailable.'},503);
  ...
  const [stages,prospectStats,customerStats,priority,activity,billing,jobs,usageStats,imageStats]=await Promise.all([
    env.DB.prepare(`SELECT stage,COUNT(*) AS count FROM prospects WHERE customer_id IS NULL GROUP BY stage`).all(),
    env.DB.prepare(`SELECT COUNT(*) AS total,SUM(CASE WHEN next_follow_up IS NOT NULL AND next_follow_up<? AND stage NOT IN ('Won','Lost') THEN 1 ELSE 0 END) AS overdue,SUM(CASE WHEN research_status='Failed' THEN 1 ELSE 0 END) AS research_failures,SUM(CASE WHEN concept_state='Build Failed' OR concept_build_error IS NOT NULL THEN 1 ELSE 0 END) AS build_failures,SUM(CASE WHEN visual_fallback=1 THEN 1 ELSE 0 END) AS fallbacks FROM prospects WHERE customer_id IS NULL`).bind(today).first(),
    ...
    env.DB.prepare(`SELECT status,COUNT(*) AS count FROM platform_jobs GROUP BY status`).all(),
    env.DB.prepare(`SELECT COUNT(*) AS calls,SUM(cache_hit) AS cache_hits,SUM(CASE WHEN provider='google_places' THEN 1 ELSE 0 END) AS google_calls,SUM(CASE WHEN provider='openai' THEN 1 ELSE 0 END) AS openai_calls FROM provider_usage_events WHERE created_at>=datetime('now','-30 days')`).first(),
    env.DB.prepare(`SELECT COUNT(*) AS attempts,SUM(CASE WHEN qa_status='approved' THEN 1 ELSE 0 END) AS approved FROM concept_images WHERE created_at>=datetime('now','-30 days')`).first().catch(()=>({attempts:0,approved:0}))
  ]);
...
export default{async fetch(request,env,context){
  ...
  try{
    ...
    if(url.pathname==='/api/admin/prospects'&&request.method==='POST'){
      await ensurePlatformSchema(env);const response=await appWorker.fetch(request,env,context);const payload=await response.clone().json().catch(()=>null);
      if(response.ok&&payload?.id)await upsertJob(env,{key:`automation:${payload.id}`,prospectId:payload.id,type:'prospect_automation',status:'Queued',stage:'Research'});
      return response;
    }
