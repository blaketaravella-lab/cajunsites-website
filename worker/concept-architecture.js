...
export function buildVerifiedBusinessProfile(p){
 ...
 return{
  ...
  sources,
  safeguards:{manual_data_wins:true,human_design_directives_win:true,only_verified_services:true,no_invented_credentials:true,no_invented_results:true}
 };
}
...
function defaultArchitecture(profile,current={}){
 ...
 const baseDesign={schema_version:'1.0',archetype,mood,image_theme:imageTheme,headline:clean(profile?.business?.summary?.value,180)||`Trusted ${vertical} in ${profile?.identity?.city?.value||'the local area'}`,cta,sections:sectionSequence,process:[],layout:'split',navigation:'minimal',content_density:'balanced',services_presentation:'service_grid',trust_strategy:['reviews','local_business_identity'],mobile_strategy:'sticky_primary_cta',imagery_strategy:['verified_service_action','representative_environment']};
 const design=applyHumanDirectives(baseDesign,current);
 const imagePlan={schema_version:'1.0',roles:[{role:'hero',purpose:'Primary conversion visual',subjects:uniq([design.image_theme||imageTheme,...services.slice(0,3)]).slice(0,6),composition:'wide landscape with headline negative space'},{role:'secondary',purpose:'Support services and trust',subjects:uniq([...services.slice(1,5),`${vertical} detail or environment`]).slice(0,6),composition:'distinct supporting landscape detail'}],universal_avoid:strategy.visual_strategy.avoid};
...
function normalizeArchitecture(raw,profile,current){
 ...
 if(current?.image_theme){const hero=image_plan.roles.find(x=>x.role==='hero');if(hero)hero.subjects=uniq([clean(current.image_theme,300),...hero.subjects]).slice(0,8)}
