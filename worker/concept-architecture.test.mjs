...
assert.equal(profile.identity_anchor.location_match_required,true);
assert.equal(profile.safeguards.manual_data_wins,true);
assert.equal(profile.safeguards.location_is_identity_constraint,true);
assert.ok(profile.profile_fingerprint,'Verified profiles must carry a deterministic fingerprint.');
...
assert.equal(architecture.architecture_version,'concept-architecture-v2');
...
assert.ok(architecture.image_plan.roles[0].avoid.some(x=>/fabricated branding/i.test(x)),'Image plan must preserve universal visual guardrails.');
...
assert.equal(customizedArchitecture.design_model.mood,'premium');
assert.equal(customizedArchitecture.design_model.layout,'editorial');
assert.equal(customizedArchitecture.design_model.headline,'Collision repair with a more refined presentation');
assert.match(customizedArchitecture.image_plan.roles[0].subjects.join(' '),/precision auto body refinishing/i);
...
assert.equal(bridged.concept_architecture_version,'concept-architecture-v2');
assert.deepEqual(bridged.concept_image_plan.roles,architecture.image_plan.roles);
assert.match(bridged.design_profile.image_theme,/Collision Repair|Auto Body Repair|Paint Refinishing/i,'Compiled image subjects must reach the existing image-generation bridge.');

const persisted={...collision,verified_business_profile_json:JSON.stringify(profile),concept_strategy_json:JSON.stringify(architecture.strategy),concept_design_model_json:JSON.stringify(architecture.design_model),concept_image_plan_json:JSON.stringify(architecture.image_plan),concept_architecture_version:'concept-architecture-v2'};
const reused=await compileConceptArchitecture({},persisted);
assert.equal(reused.source,'reused_verified_architecture','Unchanged verified profiles must reuse a stable strategy instead of redesigning on every rebuild.');
...
const missingLocation={...collision,city:null};
assert.equal((await compileConceptArchitecture({},missingLocation)).build_readiness,'needs_identity_review','Business name + city + state are required identity anchors.');
...
assert.match(compiler,/profile_fingerprint/,'Verified profiles must be fingerprinted for stable rebuild behavior.');
assert.match(compiler,/canReuse/,'Unchanged profile rebuilds must reuse strategy state.');
assert.match(compiler,/location_match_required:true/,'Location must remain part of entity identity.');
assert.match(compiler,/plannedTheme/,'Compiled image-plan subjects must be bridged into image generation.');
