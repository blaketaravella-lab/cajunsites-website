...
async function applyStoredDirectivesToResearch(env,id,directives=null){
  const p=await prospect(env,id);if(!p)return null;const proposal=normalizeProposal(directives||parseJson(p.design_directives_json,null));if(!proposal)return null;const research=parseJson(p.research_json,null);if(!research)return null;
  ...
  await env.DB.prepare('UPDATE prospects SET research_json=?,design_directives_json=?,concept_design_model_json=?,design_directives_updated_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(JSON.stringify(next),JSON.stringify(merged),JSON.stringify(merged),id).run();return merged;
}
...
async function applyProposal(request,env,user,context,id,messageId){
  ...
  const row=await env.DB.prepare("SELECT * FROM design_chat_messages WHERE id=? AND prospect_id=? AND role='assistant' LIMIT 1").bind(messageId,id).first();if(!row)return json({ok:false,error:'Design proposal not found.'},404);
  ...
  const p=await prospect(env,id);if(!p)return json({ok:false,error:'Prospect not found.'},404);if(p.research_status!=='Complete'||!p.research_json)return json({ok:false,error:'Business research must complete before Design Studio can rebuild the concept.'},409);
  const merged=await applyStoredDirectivesToResearch(env,id,proposal);await env.DB.prepare('UPDATE design_chat_messages SET applied_at=CURRENT_TIMESTAMP WHERE id=?').bind(messageId).run();
